    ...
    func send(phone: String, message: String) async throws -> TextbeltResponse {
        ...
        do {
            ...
            if decoded.success == false {
                // Textbelt returns success=false with an error string for failures/quota issues
                let message = decoded.error ?? "Unknown Textbelt error"
                throw TextbeltClientError.serviceError(message)
            }
